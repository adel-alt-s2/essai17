...
export default function Dashboard() {
  const { user } = useAuth();
  const { patients, paiements, rendezVous } = useData();

  // Calculs des statistiques
  const stats = useMemo(() => {
    const today = new Date().toISOString().split('T')[0];
    const consultationsDuJour = paiements.filter(p => p.date.includes(today));
    ...
    const revenueHier = paiements
      .filter(p => p.date.includes(hier.toISOString().split('T')[0]) && p.statut === 'Payé')
      .reduce((sum, p) => sum + parseFloat(p.montant.replace(',', '.')), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <ConsultationTable visits={paiements} />
    </div>
  );
}